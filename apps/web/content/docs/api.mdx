...
The browser-facing API is versioned under `/api/v1` on the web origin. The web Worker forwards those
requests to the API Worker through a private Service Binding; clients never call a Worker-internal
hostname.
...
| -------- | -------------------------------------------------- | ------------------------- | --------------------------------------------- |
| `GET`    | `/health`                                          | Public                    | API Worker health                             |
